...
use crate::editor::{AutosuggestionLocation, AutosuggestionType, CrdtOperation, EditorAction};
...
#[test]
fn paste_raw_image_clipboard_in_windows_codex_routes_to_agent_when_input_is_visible() {
    if !cfg!(windows) {
        return;
    }
...
#[test]
fn paste_raw_image_clipboard_from_focused_input_in_windows_codex_routes_to_agent() {
    if !cfg!(windows) {
        return;
    }
    ...
    App::test((), |mut app| async move {
        ...
        terminal.update(&mut app, |view, ctx| {
            ...
            editor.update(ctx, |editor, ctx| {
                editor.handle_action(&EditorAction::Paste, ctx);
            });
        });
